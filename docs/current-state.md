...
项目已从空仓库完成初始化，并完成阶段 1、阶段 2、阶段 2B、阶段 2C、阶段 2C.1～2C.4b、阶段 2.5、阶段 2.5A～2.5C 和阶段 2D.1。系统模型保持 `docs/system-models-v1.md` v1.1。阶段 2.5C 已将底部指挥台标准化为资源技能、英雄棋盘、主要操作三栏；20 毫秒固定步长、统一伤害内核、移动、召唤、十格和随机流均保持不变。

当前固定使用疾风猎手、余烬法师、岩盾先锋、星辉祭司组成 4 英雄池；四者的 1 星战场图已启用，3/4 星图缺失时按 4→3→1 和 3→1 回退到 1 星图。森灵唤师 1 星文件已准备但未登记 Manifest、未加载、未加入英雄定义或召唤袋。怪物、背景、头像和音频仍使用程序 fallback 或安全静默。
...
- 英雄名称、星级和真实 1 星战场图显示在所属格位；图片不可用时回退几何占位。不支持自由放置，也没有站位属性。
...
- 当前 Manifest 启用 4 个 `heroBattleImage`：疾风猎手、余烬法师、岩盾先锋、星辉祭司的 `battle-1star.png`；均为可选资源并以程序图形作为失败回退。
...
- 五张已准备英雄 PNG 均为 1254×1254 RGBA；当前只启用四名运行时英雄。每名英雄的 scale、脚底 anchor 和格位 offset 位于 `visualDefinitions.ts`；首轮人工验收后，正式纹理最大画布显示尺寸由 56px 提升到 72px，程序 fallback 尺寸不变，遮挡与边缘仍需真实 Canvas 复查。
...
- 开发构建可用 `?uiDebug=1` 一次性显示三栏、组件框、行基线、格位、英雄视觉上限和文字安全区；默认关闭且不参与任何玩法计算。
...
- `npm test`：通过；29 个测试文件、187 个用例全部通过。
- `npm run build`：通过；Vite 转换 80 个模块并生成 `dist/`。
...
- 构建提示：Phaser 完整运行库使主 JavaScript chunk 超过 Vite 默认 500 kB 提示阈值；当前主包约 1,505.73 kB、gzip 396.00 kB，不阻断构建。待真实 Canvas 与加载性能形成目标后再评估拆包。
