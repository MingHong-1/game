...
项目已从空仓库完成初始化，并完成阶段 1、阶段 2、阶段 2B、阶段 2C、阶段 2C.1～2C.4b、阶段 2.5、阶段 2.5A～2.5C.2c 和阶段 2D.1。系统模型保持 `docs/system-models-v1.md` v1.1。阶段 2.5C.2c 将 Hover 降级为弱内框、强化实例级 Selected 外框与右上菱形标记，并把统一站立横线降低到格位底部附近；11/6/7px 棋盘间距、实例选择、英雄尺寸和全部玩法规则保持不变。

当前固定使用疾风猎手、余烬法师、岩盾先锋、星辉祭司组成 4 英雄池；四者由 1254×1254 母版确定性生成的 256×256 运行纹理已启用，3/4 星图缺失时按 4→3→1 和 3→1 回退到 1 星运行纹理。森灵唤师母版和运行纹理均已准备，但未登记 Manifest、未加载、未加入英雄定义或召唤袋。怪物、背景、头像和音频仍使用程序 fallback 或安全静默。
...
│   │   ├── statistics/           # 独立于 Phaser 的整局英雄伤害统计
│   │   ├── BattleEvents.ts
│   │   ├── BattleSession.ts
│   │   ├── BattleSimulation.ts
│   │   ├── BattleState.ts
...
│   ├── main.ts
│   └── style.css
├── tools/                         # 运行纹理生成、验证、Pillow 开发依赖与报告
├── tests/
...
- 当前只排入四名运行时英雄的 1 星 PNG；森灵唤师、3/4 星、头像、怪物、背景和音频均不入队，也不依靠 404 探测。
...
- 持续显示能量、费用、英雄数量、已解锁格位和扩格进度；格位默认只显示英雄主体与必要状态，名称和精确星级在选中信息条查看。
...
- 真实 1 星运行纹理显示在所属格位；完整名称、精确星级、职业、基础攻击和暴击率显示在只读选中信息条。图片不可用时回退几何占位。不支持自由放置，也没有站位属性。
- 开发模式提供可关闭的 FPS、帧耗时、固定模拟步数、插值 alpha、丢弃追帧时间、长帧计数、敌人/弹道/Tween 数、时间、随机种子和状态信息。
- 顶部栏集中星核生命、波次、时间、敌人计数、暂停、倍速、调试和返回入口。
- 下方指挥台采用 296/592/296 的左中右三栏：左栏显示能量、费用和技能入口，中栏只承载居中的两排五格，右栏集中编制、扩格、主召唤与后续合成/重构入口。
- 波次、Boss、召唤、扩格、敌人受击和星核受伤具备轻量且有界的反馈动画。

当前四名运行时英雄使用本地真实 1 星 PNG；怪物、战场、星核、VFX 和 UI 仍由原创程序图形与文字构成。没有远程图片或音频请求，资源是否可用不改变玩法。

## 当前资源与表现框架

- `AssetManifest` 是可请求资源的唯一清单；支持图片、Sprite Sheet、Texture Atlas、背景、特效、音频和字体预留，开发阶段拒绝重复 `assetId`、重复 Phaser key、空路径、远程路径和不匹配的加载参数。
- 当前 Manifest 启用 4 个 `heroBattleImage`：疾风猎手、余烬法师、岩盾先锋、星辉祭司的 `runtime/battle-1star.png`；均为 256×256 RGBA 可选资源并以程序图形作为失败回退。1254×1254 母版不进入加载队列。
...
- 五张 1254×1254 RGBA 文件是不可覆盖的高分辨率母版；`tools/generate_hero_runtime_assets.py` 读取 Alpha 主体边界并生成五张 256×256 RGBA 运行纹理，`verify_hero_runtime_assets.py` 校验母版/输出 SHA、尺寸、模式、四角透明和安全边距。当前只启用四名运行时英雄。
- 每名英雄的整数目标显示尺寸、脚底 anchor 和格位 offset 位于 `visualDefinitions.ts`：疾风猎手为 80px，其余三名为 78px，统一最大画布显示尺寸 84px；程序 fallback 尺寸不变。最终轮廓清晰度、武器边缘和高 DPI 遮挡仍需真实 Canvas 复查。
- 当前 footAnchor 继续为疾风猎手 0.93、余烬法师 0.90、岩盾先锋/星辉祭司 0.89；slotOffset 为 `(0,39)`、`(0,36)`、`(1,35)`、`(1,35)`，只用于让完整主视觉相对格位顶边受控上溢约 12～16px。
- `HeroStarPresentationModel` 只生成 1～4 星的主视觉、分身参数和满星光环；视觉分身不具有实例 ID、输入、攻击、伤害、Buff 或统计职责。
...
- `BottomCommandLayout` 从统一令牌和关卡格位坐标派生三栏及全部组件矩形；场景不再逐个维护底部按钮、状态卡和提示坐标。
- `HeroSlotView` 默认不常驻显示完整英雄名和完整星级文案；显示树固定为底板 → HeroEcho/HeroMain → 2px 脚底前唇 → 身份框/弱 Hover/Selected 外框与标记 → 锁层 → HitArea，不对英雄使用 80×76 矩形裁切。锁层只在锁定格可见。
- `SelectedHeroInfoBar` 位于中央棋盘顶部，尺寸 576×28；从具体选中实例派生名称、精确星级、职业、有效基础攻击和暴击率，并与上下两排格位保持分离。格位 HitArea 仍为固定 80×76，不依赖 PNG 透明区域。
- 中央棋盘下排由基准 `y=158` 统一调整为 `y=153`；下排底板距中央栏底边 7px，选中外框与脚底/满星光圈均避开末端 4px 安全区。英雄显示尺寸和 `visualDefinitions` 未改变。
- 垂直间距再平衡后，上排通过统一 `firstRowVerticalAdjustment=-5` 从局部 `y=76` 调整为 `y=71`，下排保持 `y=153`。信息条底部到上排边框、两排边框之间、下排边框到中央栏底边分别为 11/6/7px；信息条与下排位置、中央栏高度和末端 4px 安全区均未改变。
- 英雄选择使用稳定局内 `instanceId`，不按 `heroId` 或数组首个匹配推断。青白 2px 外框和右上 6×6 菱形标记独立于职业色与星级色；Hover 仅为内缩 3px 的 1px/0.40 弱内框。同种英雄只高亮被选择的具体实例，失效实例、重开和场景销毁会清空选择。
- `SlotForegroundLip` 统一位于格位局部 `y=43`，宽 58px、高 2px、Alpha 0.55，距格位底边 8px；不再读取单英雄 slotOffset 决定横线位置。英雄、分身和脚底圆环均未移动，横线只形成轻量脚底前景层。
- `BattleRightDrawer` 默认收起，以 236×384 逻辑尺寸覆盖战场右侧，不永久压缩或移动战场。正式标签为怪物情报和伤害统计；开发环境或显式调试参数增加模拟调试。
- 怪物情报来自关卡波次、`EnemyDefinition` 与运行时快照；可按稳定 enemy instanceId 点击选择，选择环只作用视觉层，目标死亡或重置后自动清除。
- `BattleStatisticsTracker` 从成功应用的 `DamageResult.appliedDamage` 记录实例级伤害，并按 heroId 汇总总伤、占比、普攻、暴击、命中和模拟时间 DPS；UI 收起不停止统计，重置清空统计。
- `HeroSlotInteractionState` 统一预留拖拽/合成相关显示状态和格位 HitArea；当前没有注册拖拽输入、没有 `MergeIntent`、没有 `MergeSystem`、没有消费 `mergeRng`。
- `Panel`、`GameButton`、`ResourceDisplay`、`HeroSlotView`、`HealthBar` 和 `BattleNotice` 提供可复用显示职责。
- 底部控件采用 4/8/12/16/24px 间距令牌，常规按钮 40px、主按钮 52px、状态卡 44px；禁用按钮统一降低填充、边框和整体透明度。
- 可用 `?uiDebug=1` 显示三栏、组件框、行基线、格位 HitArea、英雄主/分身与光圈范围、选中信息条、下排安全线、选中外框/标记、当前实例 ID 和抽屉边界；默认关闭且不参与任何玩法计算。
...
- 战斗区背景、雾气、森林、岩石、道路、星核和怪物仍为原创程序图形；当前四名英雄使用本地真实 1 星 PNG，没有远程请求。
- 正式英雄图片只替换 `HeroBattleView` 的角色主体；名称、星级、格位、状态 UI、攻击和伤害逻辑不依赖图片。
...
- 装备、英雄养成、完整关卡和正式美术资源
- 英雄 3/4 星、头像、怪物、背景、特效和音频正式资源；当前四名英雄仅接入 1 星战场图
- 地图与道路空间的下一轮 UI 协调优化（阶段 2.5D）
- 森灵唤师玩法定义和召唤物系统；其 1 星图片已准备但未启用
- 浏览器本地存档
...
| 面板、按钮、资源、格位、选中信息条、右侧抽屉、生命条和提示 | `src/ui/components/` |
| UI 派生状态、按钮保护、队列和生命周期 | `src/ui/state/` |
| 怪物情报、选中英雄和伤害统计 ViewModel | `src/ui/viewmodels/`、`src/battle/statistics/` |
| 英雄运行纹理生成与验证 | `tools/generate_hero_runtime_assets.py`、`tools/verify_hero_runtime_assets.py` |
...
- `npm install`：通过；共审计 52 个包，0 个已知漏洞；`@types/node` 仅用于 Vitest 文件哈希/PNG 头测试，不进入浏览器运行包。
- `npm run typecheck`：通过。
- `npm test`：通过；32 个测试文件、226 个用例全部通过。
- `npm run build`：通过；Vite 转换 91 个模块并生成 `dist/`，主包约 1,540.71 kB、gzip 405.76 kB。
...
- 阶段 2.5A HTTP 冒烟：四张启用英雄 PNG、首页和入口模块均返回 HTTP 200；森灵唤师和 3/4 星资源不在 Manifest。真实 Canvas 尺寸、锚点、透明边缘和重开残影仍待人工验收。
- 阶段 2.5C HTTP 冒烟：Vite 在 `127.0.0.1:5178` 成功启动；首页、入口模块、`BottomCommandLayout`、`?uiDebug=1` 和英雄 PNG 均返回 HTTP 200；未进行真实 Canvas 三栏对齐与遮挡验收。
- 阶段 2.5C.2 HTTP 冒烟：Vite 在 `127.0.0.1:5179` 成功启动；首页、入口模块和四张启用的 256×256 运行纹理均返回 HTTP 200。母版与森灵唤师不在正常 Manifest 队列；未进行真实 Canvas 清晰度、抽屉和选择交互验收。
- 阶段 2.5C.2b HTTP 冒烟：Vite 在 `127.0.0.1:5181` 成功启动；首页、入口模块和四张启用运行纹理均返回 HTTP 200，日志无 404。Manifest 仍不包含母版、森灵唤师或 3/4 星资源；未进行真实 Canvas 底部间距与实例选中验收。
- 阶段 2.5C.2-spacing HTTP 冒烟：Vite 在 `127.0.0.1:5182` 成功启动；首页、入口模块和四张启用运行纹理均返回 HTTP 200，日志无 404。Manifest 未修改，母版、森灵唤师和 3/4 星资源仍不进入正常加载流程；未进行真实 Canvas 垂直间距验收。
- 阶段 2.5C.2c HTTP 冒烟：Vite 在 `127.0.0.1:5183` 成功启动；首页、入口模块和四张启用运行纹理均返回 HTTP 200，日志无 404。Manifest、母版、森灵唤师与 3/4 星资源加载边界未改变；未进行真实 Canvas Selected/Hover 与站立横线验收。
...
- 构建提示：Phaser 完整运行库使主 JavaScript chunk 超过 Vite 默认 500 kB 提示阈值；当前主包约 1,540.71 kB、gzip 405.76 kB，不阻断构建。待真实 Canvas 与加载性能形成目标后再评估拆包。

下一阶段是阶段 2.5D“地图与道路空间优化”；完成表现空间验收后继续阶段 2D.2“攻击形式与范围查询”。阶段 2D.1 已为后者提供统一伤害请求、计算结果和事件边界；状态、怪物能力和英雄具体机制仍依次留给 2D.3～2D.5。
