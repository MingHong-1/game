...
- `public/assets/heroes/wind-hunter/battle-1star.png`：已启用，对应玩法 ID `gale-hunter`。
- `public/assets/heroes/ember-mage/battle-1star.png`：已启用。
- `public/assets/heroes/stone-vanguard/battle-1star.png`：已启用。
- `public/assets/heroes/starlight-priest/battle-1star.png`：已启用。
- `public/assets/heroes/forest-summoner/battle-1star.png`：文件已准备，但未登记 Manifest、未加载、未加入运行时英雄池。

当前没有 3 星、4 星或头像资源。四张启用图均为可选资源；加载失败时只报告一次并回退程序图形，不影响召唤、攻击、伤害、星级或胜负。
...
1. 放入 `public/assets/heroes/wind-hunter/battle-1star.png`。
2. 在 Manifest 新增 `assetId=hero.gale-hunter.battle.1star`、类型 `heroBattleImage`、本地路径、唯一 Phaser key、owner 和 `fallback=programmatic`。
3. 校验后设置 `enabled=true`，分组可使用 `battle-core`。
4. 在疾风猎手 `HeroVisualDefinition.battle1StarAssetId` 引用该 assetId。
5. 执行三项工程验证。
6. 完成只启用疾风猎手的第一步自动化冒烟后，再显式启用其余三名运行时英雄。
7. 浏览器检查四张请求、脚底、格位安全区、名称、星级、武器裁切与透明边缘；森灵唤师和 3/4 星资源不得被请求。

当前视觉配置由 `src/data/visualDefinitions.ts` 管理，格位纹理最大画布显示尺寸在首轮人工验收后由 56px 调整为 72px。该配置只影响表现；程序 fallback 尺寸不随纹理上限变化，仍需真实 Canvas 复查遮挡与边缘，不应通过编辑源 PNG 或修改战斗坐标校准。
